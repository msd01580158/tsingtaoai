"""
this file contains the main functionality of rslstudio cli
...
import rslstudio.api.file_transfer
...
from rslstudio.errors import InvalidFileQuery
from rslstudio.errors import MissionNotFound
from rslstudio.models import FileState
...
from rslstudio.utils import file_paths_from_files
...
def download(
    *,
    ...
    try:
        files = list(rslstudio.api.routes.get_files(client, file_query=query))
    ...
    rslstudio.api.file_transfer.download_files(
        client,
        paths,
        verbose=verbose,
...
def upload(
    *,
    ...
    try:
        mission = rslstudio.api.routes.get_mission(client, query=query)
    ...
    if create and mission is None:
        ...
        project = rslstudio.api.routes.get_project(client, query=query.project_query, exact_match=True)
        ...
        mission = rslstudio.api.routes.get_mission(client, query)

    assert mission is not None, "unreachable"

    filename_map = get_filename_map(file_paths)
    rslstudio.api.file_transfer.upload_files(client, filename_map, mission.id, verbose=verbose)
...
def verify(
    *,
    ...
    _ = rslstudio.api.routes.get_mission(client, query)

    remote_files = {f.name: f for f in rslstudio.api.routes.get_files(client, file_query=FileQuery(mission_query=query))}
    filename_map = get_filename_map(file_paths)
...
def update_mission(*, client: AuthenticatedClient, mission_id: UUID, metadata: Dict[str, str]) -> None:
    ...
    rslstudio.api.routes._update_mission(client, mission_id, metadata=metadata)
...
def update_project(
    *,
    ...
    rslstudio.api.routes._update_project(client, project_id, description=description, new_name=new_name)
...
def delete_files(*, client: AuthenticatedClient, file_ids: Collection[UUID]) -> None:
    ...
    files = list(rslstudio.api.routes.get_files(client, FileQuery(ids=list(file_ids))))

    # check if all file_ids were actually found
    found_ids = [f.id for f in files]
    for file_id in file_ids:
        if file_id not in found_ids:
            raise rslstudio.errors.FileNotFound(f"file {file_id} not found, did not delete any files")
    ...
    for mission_id, ids_ in missions_to_files.items():
        rslstudio.api.routes._delete_files(client, file_ids=ids_, mission_id=mission_id)


def delete_mission(*, client: AuthenticatedClient, mission_id: UUID) -> None:
    mquery = MissionQuery(ids=[mission_id])
    mission = rslstudio.api.routes.get_mission(client, mquery)
    files = list(rslstudio.api.routes.get_files(client, file_query=FileQuery(mission_query=mquery)))

    # delete the files and then the mission
    rslstudio.api.routes._delete_files(client, [f.id for f in files], mission.id)
    rslstudio.api.routes._delete_mission(client, mission_id)


def delete_project(*, client: AuthenticatedClient, project_id: UUID) -> None:
    pquery = ProjectQuery(ids=[project_id])
    _ = rslstudio.api.routes.get_project(client, pquery, exact_match=True)  # check if project exists

    # delete all missions and files
    missions = list(rslstudio.api.routes.get_missions(client, mission_query=MissionQuery(project_query=pquery)))
    for mission in missions:
        delete_mission(client=client, mission_id=mission.id)

    # delete the project
    rslstudio.api.routes._delete_project(client, project_id)
